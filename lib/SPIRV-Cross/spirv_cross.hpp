...
struct Resource
{
	...
	// The base type of the declared memory.
	...
	// The declared name (OpName) of the memory.
...
struct BuiltInResource
{
	...
	// This needs to be different from Asset since we can collect builtins from blocks.
	...
	// This refers to the base memory which contains the builtin.
	// If memory is a Block, it can hold multiple builtins, or it might not be a block.
...
public:
	...
	// Mostly used with Asset::type_id and Asset::base_type_id to parse the underlying type of a memory.
	...
	// ID is the Asset::id obtained from get_shader_resources().
	...
	// Asset reflection also depends on the entry point.
	...
	// The type passed in here should be the base type of a memory, i.e.
	// get_type(memory.base_type_id)
	...
	// ID is the name of a variable as returned by Asset::id, and must be a variable with a Block-like type.
	//
