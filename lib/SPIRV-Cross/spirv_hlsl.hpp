...
// By matching stage, desc_set and binding for a SPIR-V memory,
// register bindings are set based on whether the HLSL memory is a
// CBV, UAV, SRV or Sampler. A single binding in SPIR-V might contain multiple
// memory types, e.g. COMBINED_IMAGE_SAMPLER, and SRV/Sampler bindings will be used respectively.
...
public:
	...
	// Controls how memory bindings are declared in the output HLSL.
	void set_resource_binding_flags(HLSLBindingFlags flags);

	// memory is a memory binding to indicate the HLSL CBV, SRV, UAV or sampler binding
