...
#define ADD_ARG_IDX_TO_BINDING_NUM_LOOKUP(rez) \
	...
	resource_arg_buff_idx_to_binding_number[arg_idx_tuple] = binding.binding
		...
		case SPIRType::SampledImage:
			...
			break;
		default:
			SPIRV_CROSS_THROW("Unexpected argument buffer memory base type. When padding argument buffer elements, "
...
// The returned value is retrieved from the memory binding added using add_msl_resource_binding().
                                                  ...
                                                  SPIRVariable &var, InterfaceBlockMeta &meta)
{
	...
	// Make sure to register the variables as unique memory names ahead of time.
...
{
	// Output resources, sorted by memory index & type
	...
	ir.for_each_typed_id<SPIRVariable>([&](uint32_t var_id, SPIRVariable &var) {
		    ...
		    !is_hidden_variable(var))
		{
			...
			if (var.storage == StorageClassUniform || var.storage == StorageClassStorageBuffer)
			{
				for (auto &resource : resources)
				{
					     ...
					     resource.var->storage == StorageClassStorageBuffer))
					{
						...
						// Self-reference marks that we should declare the memory,
			...
			else if (!constexpr_sampler)
			{
				...
				// Don't allocate memory indices for aliases.
...
// Returns the Metal index of the memory of the specified type as used by the specified variable.
uint32_t CompilerMSL::get_metal_resource_index(SPIRVariable &var, SPIRType::BaseType basetype, uint32_t plane)
{
	...
	{
		// Frame-buffer fetch gets its fallback memory index from the input attachment index,
	...
	// If a binding has not been specified, revert to incrementing memory indices.
	...
	else
	{
		...
		// Allocate from plain bindings which are allocated per memory type.
		switch (basetype)
		{
		case SPIRType::Image:
...
// If the variable is a Uniform or StorageBuffer, mark that a memory has been written to.
...
void CompilerMSL::analyze_argument_buffers()
{
	...
	// The [[id(N)]] values used correspond to the memory mapping we have for MSL.
	...
	if (needs_swizzle_buffer_def || needs_buffer_sizes)
	{
		...
		// We might have to add a swizzle buffer memory to the set.
...
// Return the memory type of the app-provided resources for the descriptor set,
// that matches the memory index of the argument buffer index.
// This is a two-step lookup, first lookup the memory binding number from the argument buffer index,
// then lookup the memory binding using the binding number.
MSLResourceBinding &CompilerMSL::get_argument_buffer_resource(uint32_t desc_set, uint32_t arg_idx)
{
	...
	SPIRV_CROSS_THROW("Argument buffer memory base type could not be determined. When padding argument buffer "
