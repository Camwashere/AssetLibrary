...
// Matches the binding index of a MSL memory for a binding within a descriptor set.
// Taken together, the stage, desc_set and binding combine to form a reference to a memory
...
// If the memory array is a run-time-sized array, which are legal in GLSL or SPIR-V, this value
...
// specify the base type and array size of the memory in the argument buffer, if that memory
...
// need to be populated, and if the memory is also not a run-time sized array, the count
...
// and (for iOS only) the memory is not a storage image (sampled != 2), the binding reference we
...
// become a [[buffer(N)]], [[texture(N)]] or [[sampler(N)]] depending on the memory types used.
...
public:
	...
	struct Options
	{
		...
		// - Tier2 supports writable images on macOS and iOS, and higher memory count limits.
		...
		// Aligns each memory in an argument buffer to its assigned index value, id(N),
	...
	// memory is a memory binding to indicate the MSL buffer,
	// texture or sampler index to use for a particular SPIR-V description set
	// and binding. If memory bindings are provided,
	...
	// desc_set and binding are the SPIR-V descriptor set and binding of a buffer memory
	// in this shader. index is the index within the dynamic offset buffer to use. This
	// function marks that memory as using a dynamic offset (VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER_DYNAMIC
	...
	// desc_set and binding are the SPIR-V descriptor set and binding of a buffer memory
	// in this shader. This function marks that memory as an inline uniform block
	...
	// For a variable memory ID obtained through reflection API, report the automatically assigned memory index.
	...
	// If the memory was a combined image sampler, report the image binding here,
	// use the _secondary version of this call to query the sampler half of the memory.
	...
	// sampler's binding is returned instead. For any other memory type, -1 is returned.
	...
	// in which case the second plane's binding is returned instead. For any other memory type, -1 is returned.
	...
	// in which case the third plane's binding is returned instead. For any other memory type, -1 is returned.
...
protected:
	...
	// If the underlying memory has been used for comparison then duplicate loads of that memory must be too
	// Use Metal's native frame-buffer fetch API for subpass inputs.
