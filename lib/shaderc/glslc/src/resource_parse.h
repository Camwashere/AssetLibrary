...
// Returns true when two memory setting structures are equal.
inline bool operator==(const ResourceSetting& lhs, const ResourceSetting& rhs) {
  return (lhs.limit == rhs.limit) && (lhs.value == rhs.value);
}


// Parses a memory limit setting string.  On success, returns true and populates
